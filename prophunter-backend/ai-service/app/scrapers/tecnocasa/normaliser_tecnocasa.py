...
def _extraire_transaction_et_type(url: str) -> tuple:
...
def _separer_ville(ville_brute: str) -> tuple:
    if not ville_brute:
        return None, None
...
def _normaliser_annonce(brute: dict) -> dict:
    ...
    type_transaction, type_bien = _extraire_transaction_et_type(brute.get("url"))
    surface = brute.get("surface")
    ville, quartier = _separer_ville(brute.get("ville"))
    ...
    return {
        ...
        "bien": {
            ...
            "nombre_pieces": _extraire_nombre(brute.get("pieces")),
            "nombre_chambres": _extraire_nombre(brute.get("chambres")),
            "nombre_salles_bain": _extraire_nombre(brute.get("salles_de_bain")),
...
def normaliser(annonces: list) -> list:
    ...
    resultat = [_normaliser_annonce(a) for a in annonces]

    suspectes = [a for a in resultat if a["metadonnees_scraping"]["erreurs"]]

    print(f"\n{len(resultat)} annonces normalisées")
