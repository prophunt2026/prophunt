...
def build_feature_metadata(df: pd.DataFrame) -> dict:
    ...
    for col in all_cols:
        if col == target or col in binary_features: continue
    ...
    numeric_features = [c for c in all_cols if c != target and c not in binary_features and c not in categorical_features and pd.api.types.is_numeric_dtype(df[c])]
