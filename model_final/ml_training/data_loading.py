...
def load_and_prepare_data(data_path: str, metadata: Dict[str, Any]) -> Tuple[pd.DataFrame, pd.Series, Dict[str, List[str]]]:
    ...
    Note:
        P2-B: The full_df preserves 'localisation.delegation' for use as
        GroupKFold groups in pipeline.py (Diagnostics Report §6, P2).
    """
...
def create_composite_stratifier(df: pd.DataFrame) -> pd.Series:
    ...
    if 'price_bin' not in df.columns:
        if TARGET_COL in df.columns:
            df = df.copy()
            df['price_bin'] = pd.qcut(df[TARGET_COL], q=4, labels=False, duplicates='drop').fillna(0).astype(int)
        else:
            df = df.copy()
            df['price_bin'] = 0

    is_vente = df['is_vente'].astype(str) if 'is_vente' in df.columns else pd.Series('unknown', index=df.index)
    ...
    return (is_vente + '_' + gouv + '_' + pbin).astype('category')
...
def stratified_train_val_test_split(
    ...
    Uses StratifiedKFold for balanced proportions.
