...
def validation_step1_heldout(test_metrics: Dict[str, float], raw_metrics: Dict[str, float] = None) -> Dict[str, Any]:
    """Step 1: Held-out test validation.

    P1-A: Primary targets are now in raw total-price TND space.
    Log-space/per-m² metrics kept as secondary diagnostics.
    """
    # Primary targets: raw total-price TND (aligned with business objective)
    raw_targets = {'RMSE_raw': 80000, 'MAE_raw': 50000, 'R2_raw': 0.50, 'MAPE_raw': 40, 'MedAE_raw': 35000}
    # Secondary targets: per-m² price space (original)
    pm2_targets = {'RMSE': 1500, 'MAE': 900, 'R2': 0.50, 'MAPE': 40, 'MedAE': 600}

    results = {}

    # Raw-price targets (PRIMARY)
    if raw_metrics:
        for metric, target in raw_targets.items():
            value = raw_metrics.get(metric, float('inf'))
            if metric == 'R2_raw':
                passed = value >= target
            else:
                passed = value <= target
            results[metric] = {'value': value, 'target': target, 'passed': passed, 'primary': True}

    # Per-m² targets (SECONDARY)
    for metric, target in pm2_targets.items():
        value = test_metrics.get(metric, float('inf'))
        if metric == 'R2':
            passed = value >= target
        else:
            passed = value <= target
        results[metric] = {'value': value, 'target': target, 'passed': passed, 'primary': False}
...
def run_full_validation(
    ...
    raw_metrics = eval_results['test'].get('raw_metrics', None)

    # Step 1
    logger.info("\nStep 1: Held-out test validation...")
    step1 = validation_step1_heldout(test_metrics, raw_metrics=raw_metrics)
    for metric, info in step1.items():
        status = "PASS" if info['passed'] else "FAIL"
        primary_tag = " [PRIMARY]" if info.get('primary') else ""
        logger.info(f"  {metric}: {info['value']} (target: {info['target']}) [{status}]{primary_tag}")
